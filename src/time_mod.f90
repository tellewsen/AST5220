...
contains
  ...
  function get_H(x)
    ...
    real(dp), intent(in) :: x
    real(dp)             :: get_H
    !get_H = H_0*sqrt((Omega_b+Omega_m)*a_t**-3(SELECT RIGHT VALUE IN GRID)+(Omega_r+Omega_nu)*a_t**-4(SELECT RIGHT VALUE IN GRID) + Omega_lambda)
    get_H = 0
  end function get_H
  ...
  function get_H_p(x)
    ...
    real(dp), intent(in) :: x
    real(dp)             :: get_H_p
    !get_H_p = a_t(select right value in grid)*get_H(x)
    get_H_p = 0
